...
for (let items in lunchBox) {
	if (items.includes("side")) {
		console.log(`\nOne of the sides within my lunchbox is ${lunchBox[items]}.`);
	}
}
...
function itemsAtIdx(index){
	for (let key in pantry){
		if (pantry[key][index]){
			console.log(`Items at ${index} is \t${pantry[key][index]}`);
		}
	}
}
...
function printPackageType(pkg){
	if (pantry[pkg]){
	console.log(`Items that are ${pkg}`)
	for (let key of pantry[pkg]){
		console.log(- ${key})
	}
}
}
